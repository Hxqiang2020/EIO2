...
import motion.utils.rotation_conversions as tRot
...
class Humanoid_Batch:
    ...
    def __init__(self, cfg, device = torch.device("cpu")):
        self.cfg = cfg
        self.mjcf_file = cfg.assetFileName
    ...
    def fk_rot_batch(self, poses, convert_to_mat=True, dt=1.0/50):
        
        poses = poses[None, :].clone()
        B, seq_len = poses.shape[:2]
        pose = poses[..., :len(self._parents), :]
        ...
        J = pose_mat.shape[2] - 1  # Exclude root
        body_rotations = pose_mat[:, :, 1:]
        root_rotations = pose_mat[:, :, 0:1]

        for  i in range(J):
            if self._parents[i] == -1:
                rotations_world.append(root_rotations)
            else:
                rot_mat = torch.matmul(rotations_world[self._parents[i]], torch.matmul(self._local_rotation_mat[:,  (i):(i + 1)], body_rotations[:, :, (i - 1):i, :]))
                rotations_world.append(rot_mat)
    ...
    def ue_fk(self, joints_global_rot, root_trans, convert_to_mat=True, return_full = False, dt=1.0/50):
        device, dtype = joints_global_rot.device, joints_global_rot.dtype
        joints_global_rot_input = joints_global_rot.clone()
        B, seq_len = joints_global_rot.shape[:2]
        joints_global_rot = joints_global_rot[..., :len(self._parents), :]
